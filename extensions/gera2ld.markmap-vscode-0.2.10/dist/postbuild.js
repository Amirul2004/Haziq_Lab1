...
async function fetchAssets(assetsDir) {
  ...
  await Promise.all(paths.map(path$1 => downloadAsset(path.resolve(assetsDir, path$1), transformer.urlBuilder.getFullUrl(path$1, fastest))));
}
async function downloadAsset(fullPath, url) {
  ...
  await promises.mkdir(path.dirname(fullPath), {
    recursive: true
  });
  await promises$1.pipeline(res.body, fs.createWriteStream(fullPath));
}
